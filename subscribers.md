...
This behaviour is **compliant with both RSS and Atom specifications**. If we’re not able to find a unique ID, Superfeedr generates one, using complex feed-specific rules. (We indicate when the `<id>` has been [generated by Superfeedr](/schema.html#generated_ids) using the `generated_ids` field in the `status`).
